...
{
    return mEnabledState.contains(type) && mEnabledState[type];
}

bool Constraints::isUnited(VariableType type) const
{
    return mUnitedState.contains(type) && mUnitedState[type];
}

bool Constraints::isMultiplied(VariableType type) const
{
    return mMultipliedState.contains(type) && mMultipliedState[type];
}
...
{
    if (mMultipliedState.contains(type) && mMultipliedState[type])
    {
...
{
    if (mUnitedState.contains(type) && mUnitedState[type])
    {
        QString typeName = magic_enum::enum_name(type).data();
